...
router.get('/search', async function(req, res, next) {
  ...
  let sqlquery = "SELECT id, author, title, description, content, url, imageUrl, published_at, created_at, updated_at FROM news WHERE " + whereQueries + " LIMIT 30"
...
router.get('/comments', async function(req, res, next) {
  ...
  let commentQuery = "SELECT user_id, news_id, content, created_at, updated_at FROM comments where user_id = ?"
...
router.post('/comments', async function(req, res, next) {
  ...
  updateResponse(201, "Your comment is successfully created!")
  return res.json(apiResponse)
})
...
router.get('/my_news', async function(req, res, next) {
    try {
      ...
      let sqlquery = "SELECT news.id as id, news.author as author, news.title as title, news.description as description, news.content as content, news.url as url, news.imageUrl as imageUrl, news.published_at as published_at, news.created_at as created_at, news.updated_at as updated_at, comments.content as comment FROM news inner join comments on comments.news_id = news.id where comments.user_id = ? ORDER BY comments.updated_at DESC LIMIT 30"
      // execute sql query
